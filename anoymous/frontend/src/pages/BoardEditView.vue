<template>
  <div class="board-form-card" v-if="post">
    ...
    <form @submit.prevent="updatePost">
      ...
      <button type="submit" class="form-btn">수정 완료</button>
    </form>
  </div>
</template>
...
const updatePost = async () => {
  ...
  await axios.put(`http://localhost:8080/api/posts/${post.value.id}`, formData, {
    headers: {
  ...
  alert('수정되었습니다.')
  router.push(`/board/${post.value.id}`)
}
</script>
